...
<p>
<?php
if ( trim( IIRS_0_setting('lang_code') ) == FALSE ) {
  if ( IIRS_0_language_is_supported() ) { ?>
    The plugin should be displaying in the same language as this administration suite, <strong><?php print(IIRS_0_locale()); ?></strong>.
    Contact <?php print( IIRS_EMAIL_TEAM_LINK ); ?> if there are problems.
    You can force the plugin to display in a different language using the <a href="#IIRS_0_options_area">options area</a> below.
  <?php } else { ?>
...
if ( trim( IIRS_0_setting( 'region_bias' ) ) == FALSE ) {
  ...
  <p>
    You need to select the correct "<i>region_bias</i>" in the <a href="#IIRS_0_options_area">options area</a> below. Note that this is a bias, not a limit.
  </p>
  <?php
  }
}
